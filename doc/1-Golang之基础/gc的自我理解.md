...
```
#### 2-2 局限性
* 上面这段伪码逻辑就是写屏障, 我们知道,黑色对象的内存槽有两种位置, 栈和堆,栈空间的特点是容量小,但是要求相应速度快,因为函数调用返回频繁使用, 所以“插入屏障”机制,在栈空间的对象操作中不使用. 而仅仅使用在堆空间对象的操作中.
* 解决方法：对于栈空间不执行操作，等到堆空间标记黑色完毕之后，对栈空间进行传统STW的三色标记

#### 2-3 总结
* 如果是纯粹的插入写屏障是满足强三色不变式的（永远不会出现黑色对象指向白色对象）；
* **但是**由于栈上对象无写屏障（不 hook），那么导致黑色的栈可能指向白色的堆对象，所以必须假定赋值器（mutator）是灰色赋值器，扫描结束之后，必须 STW 重新扫描栈才能确保不丢对象；
...
* 方法二：起始快照整栈跨找，扫黑，使得整个堆上的在用对象都处于灰色保护；**整栈扫黑 ！！！**，那么在用的堆上的对象是一定处于灰色堆对象的保护下的，之后配合堆对象删除写屏障就能保证在用对象不丢失。**(全局STW)**
...
#### 4-3 万一栈上的对象A引用了堆上的对象B，由于栈区不触发混合写屏障机制，那对象8一直是白色的，最后不就被垃圾回收走了么，谁来保护它？
